...
public class RTSCamera : MonoBehaviour {
	...
	if(panInput >0)
		{
			...
			targetPos += Vector3.Cross(transform.right , Vector3.up) 
	...
	void HandleCameraDistance()
	{
		...
		{
		 destination = Vector3.Normalize(transform.position - hit.point)
	...
	void Zoom()
	{
		...
		position.distanceFromGround = 
		Mathf.Lerp (position.distanceFromGround, position.newDistance, position.zoomSmooth * Time.deltaTime);
	...
	void Rotate()
	{
		...
		{
			orbit.yRotation +=
				(currentMousePos.x - previousMousePos.x ) * orbit.yOrbitSmooth * Time.deltaTime;
		}
		transform.rotation =
			Quaternion.Euler (orbit.xRotation, orbit.yRotation, 0);
	}
